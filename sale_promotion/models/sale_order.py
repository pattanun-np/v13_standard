    ...
    def button_dump_sale_promotion(self):
        ...
        for order in self:
            if order.sale_promotion_id:
                ...
                category_rule_ids = []
                product_rule_ids = []
                if not self.sale_promotion_id:
                    promotion_ids = self.env['sale.promotion'].search([])
                else:
                    promotion_ids = self.sale_promotion_id

                for obj in promotion_ids:
                    for promo_lines in obj.item_ids:
                        if not promo_lines.date_start or str(promo_lines.date_start) <= datetime.strftime(date_order,
                                                                                                          '%Y-%m-%d'):
                            if not promo_lines.date_end or str(promo_lines.date_end) >= datetime.strftime(date_order,
                                                                                                          '%Y-%m-%d'):
                                if promo_lines.applied_on == 'product_category':
                                    categ_val = {'rule': promo_lines,
                                                 'category': promo_lines.categ_id.id,
